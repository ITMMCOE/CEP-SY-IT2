            ...
            <DropdownMenuTrigger asChild>
              <button className="flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-sidebar-foreground transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground w-full">
                <Avatar className="h-8 w-8">
                  {userAvatar && <AvatarImage src={userAvatar.imageUrl} alt="Authority avatar" />}
                  <AvatarFallback>A</AvatarFallback>
                </Avatar>
                <span className="truncate group-data-[collapsible=icon]:hidden">Authority User</span>
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent side="right" align="start">
              <DropdownMenuLabel>Authority Account</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuItem>
      ...
      <SidebarInset>
        <header className="flex h-14 items-center gap-4 border-b bg-background/95 px-6 backdrop-blur-sm sticky top-0 z-30">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-semibold">Authority Dashboard</h1>
          </div>
        </header>
